...
[ "$(id -u)" -ne 0 ] || err "Lance 'bash install.sh' avec ton utilisateur normal, pas avec sudo."
...
[ -x "$OPENCODE_BIN" ] || err "opencode est introuvable apres l'installation."
ok "opencode prêt ($($OPENCODE_BIN --version))."
...
# The service does not load ~/.bashrc, so persist the exact executable path.
if grep -q '^OPENCODE_BIN=' "$ENV_FILE"; then
    sed -i "s|^OPENCODE_BIN=.*|OPENCODE_BIN=$OPENCODE_BIN|" "$ENV_FILE"
else
    echo "OPENCODE_BIN=$OPENCODE_BIN" >> "$ENV_FILE"
fi
chmod 600 "$ENV_FILE"
